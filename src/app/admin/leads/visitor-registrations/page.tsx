...
export const metadata: Metadata = { title: "Visitor Registrations", robots: { index: false } };
...
export default async function VisitorRegistrationsPage() {
  ...
  const leads: Lead[] = conn
    ? JSON.parse(JSON.stringify(await VisitorRegistration.find().sort({ createdAt: -1 }).lean()))
    : [];

  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-extrabold text-brand-dark">Visitor Registrations</h1>
        <Link
          href="/api/admin/export/visitor-registrations"
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-brand-border bg-white">
            ...
            {leads.map((lead) => (
              <tr key={lead._id} className="align-top">
                ...
                <td className="px-4 py-3">
                  <p className="font-semibold text-brand-dark">{lead.firstName} {lead.lastName}</p>
                  <p className="text-xs text-brand-body">{lead.email}</p>
                  <p className="text-xs text-brand-body">{lead.mobile}</p>
                </td>
                <td className="px-4 py-3">{lead.company}</td>
                <td className="px-4 py-3">{lead.country}, {lead.city}</td>
                <td className="px-4 py-3">{lead.purposeOfVisit}</td>
                <td className="px-4 py-3">
                  <StatusSelect collection="visitor" id={lead._id} status={lead.status} />
                </td>
                <td className="px-4 py-3">
                  <LeadRowActions
                    collection="visitor"
        ...
        {leads.length === 0 ? (
          <p className="p-8 text-center text-sm text-brand-body">No visitor registrations yet.</p>
        ) : null}
      </div>
    </div>
  );
